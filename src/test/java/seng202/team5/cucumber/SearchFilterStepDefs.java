...
public class SearchFilterStepDefs {
    ...
    public void theUserSearchesForInTheSearchBar(String search) {
        String query = wineDAO.queryBuilder(search, "0", "0", "0", "0",0.0, 800.0, 0.0, 100.0, false);
        filteredWines = wineDAO.executeSearchFilter(query, null);
    }
    ...
    public void theUserAppliesVarietyFilter(String variety) {
        String query = wineDAO.queryBuilder("0", variety, "0","0", "0", 0.0, 800.0, 0.0, 100.0, false);
        filteredWines = wineDAO.executeSearchFilter(query, null);
    }
    ...
    public void theUserAppliesMinimumPriceRangeAndMaximumPrice(int minPrice, int maxPrice) {
        String query =
                wineDAO.queryBuilder("0", "0","0", "0", "0", minPrice, maxPrice, 0.0, 100.0, false);
        filteredWines = wineDAO.executeSearchFilter(query, null);
    ...
    public void theUserAppliesaYearFilterMinimumRatingFilterOf(int year, int rating) {
        String query =
                wineDAO.queryBuilder("0", "0", "0", "0", String.valueOf(year), 0, 800.0, 0.0, rating,
                        false);
        filteredWines = wineDAO.executeSearchFilter(query, null);
    }
    ...
    public void theUserAppliesFiltersVarietyRegionNameYearMinPriceMaxPriceMinRatingSearchBar(
            ...
            String search) {
        String query = wineDAO.queryBuilder(search, variety, colour,  region, String.valueOf(year), minPrice,
                maxPrice, 0, minRating, false);
        filteredWines = wineDAO.executeSearchFilter(query, null);
    }

    @Then("the system displays results of wines with Variety {string}, colour {string}, Region {string}, Year {int}, Price between {int} and {int}, Rating of {int} or higher, and containing {string} in the name")
    public void theSystemDisplaysResultsOfWinesWithVarietyColourRegionYearPriceBetweenAndRatingOfOrHigherAndContainingInTheName(
            String variety, String colour, String region, int year, int minPrice, int maxPrice, int minRating,
            String search) {
        for (Wine wine : filteredWines) {
    ...
    public void theUserAppliesFiltersVarietyAndRegion(String variety, String region) {
        String query = wineDAO.queryBuilder("", variety, "0", region, "0", 0.0, 800.0, 0, 100.0, false);
        filteredWines = wineDAO.executeSearchFilter(query, null);
    }
